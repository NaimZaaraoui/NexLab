  ...
  return (
    <article className="bento-panel overflow-hidden">
      <div className="flex items-center justify-between border-b px-5 py-4">
        ...
        <span className="text-xs text-[var(--color-text-secondary)]">50 derniers mouvements</span>
      </div>
      {item.movements.length === 0 ? (
        <div className="px-5 py-10 text-center text-sm text-[var(--color-text-secondary)]">Aucun mouvement enregistré.</div>
      ) : (
        <div className="divide-y">
          {item.movements.map((movement) => (
            <div key={movement.id} className="grid grid-cols-[1.1fr_0.9fr_0.8fr_1fr_1fr] gap-3 px-5 py-3 text-sm">
              ...
              <div className="text-[var(--color-text-secondary)]">{movement.lotNumber || '—'}</div>
              <div className="text-[var(--color-text-secondary)]">{movement.performedBy}</div>
            </div>
          ))}
        </div>
      )}
