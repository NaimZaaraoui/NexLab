  ...
  return (
    <section className="rounded-[2rem] border border-amber-100/50 bg-amber-50/30 px-6 py-6 shadow-sm ring-1 ring-amber-900/5 transition-all">
      <div className="flex flex-col gap-2">
        <h2 className="text-sm font-semibold uppercase tracking-[0.08em] text-slate-700">Priorités du jour</h2>
      ...
      <div className="mt-4 grid gap-2">
        {items.map((item) => (
          ...
          >
            ...
            <span className="text-xs text-[var(--color-text-secondary)]">{formatReason(item)}</span>
          </Link>
        ))}
      </div>
    </section>
