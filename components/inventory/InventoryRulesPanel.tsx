...
export function InventoryRulesPanel({ item, isAdmin, onAdd, onDelete }: InventoryRulesPanelProps) {
  return (
    <article className="bento-panel overflow-hidden">
      ...
      {item.rules.length === 0 ? (
        <div className="px-5 py-10 text-center text-sm text-[var(--color-text-secondary)]">Aucun test lié pour l’instant.</div>
      ) : (
          ...
          {item.rules.map((rule) => (
            <div key={rule.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3">
              <div>
                ...
                <div className="text-xs text-[var(--color-text-secondary)]">{rule.test.code}</div>
              </div>
