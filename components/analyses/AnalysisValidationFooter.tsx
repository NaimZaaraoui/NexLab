      ...
      {!isFinalValidated ? (
        <div className="flex flex-wrap items-center gap-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3 w-fit">
          ...
          <div className="flex items-center gap-3">
            <div className={`flex h-8 w-8 items-center justify-center rounded-md text-xs font-semibold ${getAnalysisStatusMeta(analysis.status).stepNumber >= 1 ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-[var(--color-text-secondary)]'}`}>1</div>
            <div className="flex flex-col min-w-[120px]">
              ...
              {getAnalysisStatusMeta(analysis.status).stepNumber >= 2 ? (
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-[var(--color-text-secondary)] font-medium leading-none mt-0.5">Par {analysis.validatedTechName || 'Technicien'}</span>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  {canTech && analysis.status === 'in_progress' ? (
                    <button
                      ...
                      className="btn-primary-sm !h-7 !rounded-md !px-3 !text-xs disabled:cursor-not-allowed disabled:opacity-50"
                      title={
                        hasSpecimenBlockers
          ...
          <div className="flex items-center gap-3">
            <div className={`flex h-8 w-8 items-center justify-center rounded-md text-xs font-semibold ${analysis.status === 'validated_tech' ? 'bg-indigo-600 text-white' : isFinalValidated ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-[var(--color-text-secondary)]'}`}>2</div>
            <div className="flex flex-col min-w-[120px]">
              ...
              {isFinalValidated ? (
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-[var(--color-text-secondary)] font-medium leading-none mt-0.5">Par {analysis.validatedBioName || 'Biologiste'}</span>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  {canBio && analysis.status === 'validated_tech' ? (
                    <button onClick={() => handleValidation('bio')} disabled={validating} className="h-7 rounded-md bg-emerald-600 px-3 text-xs font-medium text-white transition-colors hover:bg-emerald-700">
                      Signer
                    </button>
                  ) : (
                    <span className="text-xs font-semibold text-slate-500 italic">
        ...
        <div className="flex items-center gap-3 rounded-xl border border-emerald-200/70 bg-emerald-50 p-2.5 w-fit">
          ...
          <div className="flex flex-col mr-4">
            <span className="text-xs font-bold text-emerald-600 uppercase tracking-[0.08em] leading-none mb-1">Dossier Validé & Signé</span>
            <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
              <span>{analysis.validatedBioName}</span>
              <span className="text-emerald-300">•</span>
              <span className="text-[var(--color-text-secondary)]">{analysis.validatedBioAt ? format(new Date(analysis.validatedBioAt), 'dd MMM yyyy HH:mm', { locale: fr }) : ''}</span>
            </div>
          </div>
        </div>
      )}
