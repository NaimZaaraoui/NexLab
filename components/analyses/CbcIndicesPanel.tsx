  ...
  return (
    <div className={isPrint ? 'px-1 py-2' : 'py-2'}>
      <div className={isPrint ? 'mb-5 flex items-center gap-3' : 'mb-5 flex items-center gap-3'}>
        ...
        <div>
          <h3 className={isPrint ? 'text-sm font-black uppercase tracking-[0.22em] text-black' : 'text-sm font-semibold uppercase tracking-[0.08em] text-[var(--color-text)]'}>
          ...
          >
            <div className={isPrint ? 'mb-3 flex items-center gap-3' : 'mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between'}>
              <div className="flex items-center gap-2">
                <span className={isPrint ? 'text-[11px] font-black uppercase tracking-[0.1em] text-black/70' : 'text-xs font-semibold uppercase tracking-[0.08em] text-[var(--color-text-secondary)]'}>
                  {GROUP_LABELS[group]}
                </span>
                {!isPrint && (
                  <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-[var(--color-surface-muted)] text-[var(--color-text-secondary)]" title={GROUP_HELP[group]}>
                    <Info size={12} />
                  </span>
                )}
              </div>
              {!isPrint && <p className="text-xs text-[var(--color-text-secondary)]">{GROUP_HELP[group]}</p>}
            </div>

            <table className="w-full border-collapse">
              <thead>
                <tr className={isPrint ? 'bg-black/5' : 'bg-[var(--color-surface-muted)]/60'}>
                  <th className="py-2.5 pl-3 text-left text-xs font-black uppercase tracking-[0.08em] text-slate-500 print:text-black/70">Indice</th>
                  <th className="py-2.5 text-left text-xs font-black uppercase tracking-[0.08em] text-slate-500 print:text-black/70">Valeur</th>
                  <th className="py-2.5 text-left text-xs font-black uppercase tracking-[0.08em] text-slate-500 print:text-black/70">Référence</th>
                  <th className="py-2.5 pr-3 text-left text-xs font-black uppercase tracking-[0.08em] text-slate-500 print:text-black/70">Interprétation</th>
                </tr>
              </thead>
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="even:bg-[var(--color-surface-muted)]/30 print:even:bg-black/2">
                    ...
                    <td className="py-3 pr-3 align-top max-w-[180px]">
                      <span className={item.isAlert ? 'text-xs font-bold text-rose-700 print:text-black' : 'text-xs font-semibold text-[var(--color-text-secondary)] print:text-black/70'}>
                        {item.interpretation}
                      </span>
                    </td>
                  </tr>
      ...
      <p className={isPrint ? 'mt-6 text-xs font-medium text-black/55 leading-relaxed' : 'mt-4 text-xs text-[var(--color-text-secondary)]'}>
