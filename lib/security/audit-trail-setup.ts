...
import { getDatabaseFilePath } from '@/lib/db/database-backups';
...
export async function installAuditImmutabilityTriggers() {
  ...
  try {
    // Check if the required tables exist before creating triggers
    const tablesCheck = await client.execute(`
      SELECT name FROM sqlite_master 
      WHERE type = 'table' AND name IN ('audit_logs', 'audit_logs_archive')
    `);
    
    const foundTables = new Set(tablesCheck.rows.map(r => r.name));
    if (!foundTables.has('audit_logs') || !foundTables.has('audit_logs_archive')) {
      console.log('[NexLab] Audit tables not yet ready, skipping immutability triggers setup.');
      return;
    }
